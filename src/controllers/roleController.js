...
module.exports = {
  ...
  createRole: async (req, res, next) => {
    try {
      let response = await roleService.createRole(req.body, req.user);
  ...
  getRole: async (req, res, next) => {
    try {
      ...
      let response = await roleService.getRole(id);
      res.status(response.status).send(response);
    } catch (err) {
      next(err);
    }
  ...
  updateRole: async (req, res, next) => {
    try {
      let id = req.params.id;
      let response = await roleService.updateRole(req.body, req.user, id);
      res.status(response.status).send(response);
    } catch (err) {
      next(err);
    }
   ...
   *     responses:
   *       200:
   *         description: Role Details deleted Successfully.
   *         content:
   *           application/json:
   *             schema:
   *               type: object
